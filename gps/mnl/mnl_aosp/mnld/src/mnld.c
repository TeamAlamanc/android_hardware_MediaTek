	...
	for (ne = 0; ne < nevents; ne++) {
		...
		if ((events[ne].events & EPOLLIN) != 0) {
			...
			if (fd == obj->mnl_rcv_mtklogger_fd){//from mtklogger							
				...
				MND_MSG("mtklogger client fd:%d",client_fd);
		...
		epoll_del(obj->epoll_fd,events[ne].data.fd);
		MND_ERR("wait: (%d %d %d %d %d),but %d is not in these fds,remove it from epoll_wait!", obj->cur_accept_socket,
		obj->sig_rcv_fd, obj->mnl_rcv_agps_fd, obj->mnl_rcv_mtklogger_fd, client_fd, events[ne].data.fd);
	}
